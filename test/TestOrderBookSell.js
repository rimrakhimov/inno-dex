...
contract("SellOrderBook", async accounts => {
    const orderBookType = 1; // Sell
    ...
    it("should return correct spot price", async () => {
        ...
        assert.equal(spotPrice, 2, "Spot price should not change after inserting order with price 3");

        await orderBookInstance.add(order1, { from: accounts[0] });
        spotPrice = await orderBookInstance.getSpotPrice.call();
        assert.equal(spotPrice, 1, "Spot price should decrease after inserting order with price 1");
    });
